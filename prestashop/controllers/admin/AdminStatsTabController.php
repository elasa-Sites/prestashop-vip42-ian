...
abstract class AdminStatsTabControllerCore extends AdminPreferencesControllerCore
{
    ...
    public static function displayCalendarForm($translations, $token, $action = null, $table = null, $identifier = null, $id = null)
    {
        ...
        //Edited by Danoosh - iPresta.ir - to convert date
        $tpl->assign(array(
            ...
            'datepickerFrom' => Tools::getValue('datepickerFrom', local_date('Y-m-d',strtotime($context->employee->stats_date_from))),
            'datepickerTo' => Tools::getValue('datepickerTo', local_date('Y-m-d',strtotime($context->employee->stats_date_to)))
        ));
    ...
    public function postProcess()
    {
		...
		//$ex = exploded date
		if ( Tools::getValue('persiandatepicker') && count(Tools::getValue('persiandatepicker')) > 0 && strtolower($this->context->language->iso_code == 'fa'))
		{	
