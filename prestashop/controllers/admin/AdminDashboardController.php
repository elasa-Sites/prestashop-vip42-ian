...
class AdminDashboardControllerCore extends AdminController
{
    ...
    public function renderView()
    {
        ...
        $calendar_helper->setDateFrom(Tools::getValue('date_from', local_date('Y-m-d',strtotime($this->context->employee->stats_date_from))));
        $calendar_helper->setDateTo(Tools::getValue('date_to', local_date('Y-m-d',strtotime($this->context->employee->stats_date_to))));

        $stats_compare_from = local_date('Y-m-d',strtotime($this->context->employee->stats_compare_from));
        $stats_compare_to = local_date('Y-m-d',strtotime($this->context->employee->stats_compare_to));
        ...
        $this->tpl_view_vars = array(
            'date_from' => local_date('Y-m-d',strtotime($this->context->employee->stats_date_from)),
            'date_to' => local_date('Y-m-d',strtotime($this->context->employee->stats_date_to)),
            'hookDashboardZoneOne' => Hook::exec('dashboardZoneOne', $params),
            'hookDashboardZoneTwo' => Hook::exec('dashboardZoneTwo', $params),
            //'translations' => $translations,
            'action' => '#',
            'warning' => $this->getWarningDomainName(),
            'new_version_url' => Tools::getCurrentUrlProtocolPrefix()._PS_API_DOMAIN_ALT_.'/version/check_version.php?v='._PS_VERSION_.'&lang='.$this->context->language->iso_code.'&autoupgrade='.(int)(Module::isInstalled('autoupgrade') && Module::isEnabled('autoupgrade')).'&hosted_mode='.(int)defined('_PS_HOST_MODE_'),
    ...
    public function ajaxProcessGetBlogRss()
    {
        ...
        if (!$this->isFresh('/config/xml/blog-'.$this->context->language->iso_code.'.xml', 86400)) {
            if (!$this->refresh('/config/xml/blog-'.$this->context->language->iso_code.'.xml', _PS_API_URL_ALT_.'/rss/blog/blog-'.$this->context->language->iso_code.'.xml')) {
