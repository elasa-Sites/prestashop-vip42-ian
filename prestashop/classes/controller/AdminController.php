...
class AdminControllerCore extends Controller
{
    ...
    {
		//Added by Danoosh
        $this->processConvertLocalDatePickers();
    ...
    /**
     * Added by Danoosh Miralayi - iPresta.ir - to hack values sent by "fa" datepicker
     * if lang = fa then convert persian jalali date to standard gregorian
     */
    public static function processLocalDatePickers()
    {
        if ( Tools::getValue('persiandatepicker') && count(Tools::getValue('persiandatepicker')) > 0 && Context::getContext()->language->iso_code == 'fa') {
            foreach (Tools::getValue('persiandatepicker') as $datefield) {
				$datefield = str_replace('local_', '', $datefield);
                if (preg_match('/\[\d+\]/', $datefield, $matches))
                    $datefield = preg_replace('/\[\d+\]/', '', $datefield);
                $datetime = Tools::getValue($datefield);
                if (count($matches)) {
                    $id = preg_replace(array('/\[/', '/\]/'), '', $matches[0]);
                    $datetime = $datetime[$id];
                }
                $gdate = convertLocalDate($datetime);
                if (isset($id))
                    $_POST[$datefield][$id] = $gdate;
                else
                    $_POST[$datefield] = $gdate;
            }
        }
    }
    ...
    /**
     * Added by Danoosh Miralayi - iPresta.ir - to hack values sent by "fa" datepicker
     * if lang = fa then convert gregorian standard dates to persian jalali dates
     */
    public static function processConvertLocalDatePickers()
    {
        if (Tools::getValue('persiandatepicker') && count(Tools::getValue('persiandatepicker')) > 0 && Context::getContext()->language->iso_code == 'fa') {
            foreach (Tools::getValue('persiandatepicker') as $datefield) {
				$datefield = str_replace('local_', '', $datefield);
                ...
                $jdate = local_date('Y-m-d' . (isset($date_time[1]) ? ' H:i:s' : ''), strtotime($datetime));
                if (isset($id))
                    $_POST[$datefield][$id] = $jdate;
                else
                    $_POST[$datefield] = $jdate;
            }
        }
    }
}
